...
WORKDIR /usr/local/bin/scriptNode

# Copier le package.json dans le répertoire du conteneur
COPY ./scriptsNode/package.json /usr/local/bin/scriptNode/package.json

COPY ./scriptsNode/.SECURE /usr/local/bin/scriptNode/.SECURE

# Installer les dépendances avec yarn
RUN yarn install --cwd /usr/local/bin/scriptNode  # Utiliser yarn pour installer les dépendances

# Copier le script Node.js (botTwitch.js) dans le conteneur
COPY ./scriptsNode/botTwitch.js /usr/local/bin/scriptNode/botTwitch.js
...
CMD ["bash", "-c", "yarn start /usr/local/bin/scriptNode/botTwitch.js & apache2-foreground"]
